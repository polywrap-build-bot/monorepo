...
export class CancelablePromise<T = any> extends CancelablePromiseInternal<T> {
  ...
  static resolve = function resolve(value: any) {
    return createCancelablePromise(Promise.resolve(value));
  } as CancelablePromiseOverloads['resolve'];

  static reject = function reject(reason) {
    return createCancelablePromise(Promise.reject(reason));
...
export function createCancelablePromise<T = any>(promise: Promise<T>): CancelablePromise<T> {
